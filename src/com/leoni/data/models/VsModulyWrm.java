...
public class VsModulyWrm implements Comparable{
    ...
    public int compareTo(Object o) {
        ...
        if(this.getVs().getNazov() == null){
            if(vs.getPopis() == null)
                return 0; //equal
            else
                return 1;} // null is before other strings
        else // this.member != null
        {if(vs.getVs().getNazov() == null)
            return -1;  // all other strings are after null
        else return this.getVs().getNazov().compareTo(vs.getVs().getNazov());
               /* {
                    if(this.getVs().getNazov().equals(vs.getVs().getNazov())){
                        if (this.getModuls() != null && vs.getModuls()!= null) {return this.getModuls().getSachNrBest().compareTo(vs.getModuls().getSachNrBest());}
                        else return 4;
                    }
                    else
                    return this.getVs().getNazov().compareTo(vs.getVs().getNazov());
                }*/
