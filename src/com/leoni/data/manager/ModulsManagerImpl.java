...
import com.leoni.data.criterion.In;
...
public class ModulsManagerImpl extends GenericManagerImpl<Moduls> implements ModulsManager
    ...
    public Moduls addNewModul(String sachNrBest,String sachNrLieferant,String prodGruppe,String kabelSatz,
        ...
        newModul.setAliasList(aliasList);
        newModul.setFoamForm(foamForm);
        newModul.setCarBody(carBody);
        newModul.setColor(color);
        newModul.setDifficulty(difficulty);
        ...
        newModul.setAssTime(Double.valueOf(assTime));
    ...
    public Moduls saveEditedModul(Moduls modul, String user){
        ...
        public boolean containsDiffModul(List<Lpab64> modulsList) {
            boolean containsDiffModul = false;
            List<String> modulList = new ArrayList();
            for (Lpab64 lpab64 : modulsList){
                modulList.add(lpab64.getSachNrLieferant().trim());
            }
            CriteriaAppender[] criteriaAppenders = {new Equal("difficulty", 2),new In("sachNrLieferant", modulList)};
            if (!find(Arrays.asList(criteriaAppenders)).isEmpty()) {
                containsDiffModul = true;
            }
           return  containsDiffModul;
        }
