...
public class JitAbruManagerImpl  extends GenericManagerOldImpl<JitAbru> implements JitAbruManager{
    ...
    public List<CriticalNewModul> checkForModules(String modulsString,Map<String,Set<String>> modulsInProdNr,
                                                  String dfueNr, String note){
    ...
    public List<String> getProdNrList( String date){
        String queryStr = "SELECT distinct  jitAbru.prodNr, jitAbru.kdNrDfue " +
                "FROM JitAbru jitAbru " +
                "WHERE jitAbru.dfueDatumEin =" + date;
    ...
    List<String> prodnrList = new ArrayList();
    while (results.next()) {
        //System.out.println (results.get(0) + "   " + results.get(1));
        prodnrList.add((String) results.get(0));}
    return prodnrList;
    }

    @Override
    public String controlAbruf(String prodNr) {
            List<JitAbru> jitAbruList = new ArrayList<>();
            if(prodNr != null && !prodNr.trim().equals("")){
            jitAbruList = getSessionFactory().getCurrentSession().createCriteria(JitAbru.class)
                    .add(Restrictions.eq("prodNr", prodNr.trim()))
                    .add(Restrictions.eq("pabSatzart", "662"))
                    .add(Restrictions.eq("staUebe", "0")).list(); }
            if (!jitAbruList.isEmpty()){
                return jitAbruList.get(0).getDfueNrEin();}
            else return null;
    }
}
