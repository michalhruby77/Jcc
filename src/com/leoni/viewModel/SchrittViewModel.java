...
public class SchrittViewModel {
    ...
    @Command
    @NotifyChange ({"schrittList","selectedSchritt"})
    public void search()
    {    if (id.equals("")&&sachNrLieferantSearch.equals("")&&sachNrBestSearch.equals("")&&ausfuehrungSearch.equals("")&&
        ...
        schrittList=schrittManager.getAll();}
    else schrittList = schrittManager.findBy(id,sachNrLieferantSearch,sachNrBestSearch,ausfuehrungSearch,prodGruppeSearch,
                bandNameSearch,seiteSearch,schrittSearch);
        selectedSchritt = null;
    }

    @Command
    @NotifyChange ({"schrittList","id","sachNrLieferantSearch","sachNrBestSearch","ausfuehrungSearch","prodGruppeSearch",
            "bandNameSearch","seiteSearch","schrittSearch","selectedSchritt"})
    ...
    @Command
    @NotifyChange({"schrittList","sachNrBestModul","sachNrLieferantModul","variante1","variante2","variante3",
            "variante4","seite","schritt","popis","popis","farba","isCheck","bandName","isCheckBool","assLevelSeq"})
    public void generateNewSchritt() {
        if(sachNrBestModul!=null&&sachNrLieferantModul!=null&&seite!=null&&schritt!=null&&bandName!=null && assLevelSeq != null &&
                (popis == null || popis.trim().equals("") || popis.length() <= 40)
                ){
         ...
         if(!modulsManager.findBySachNrLieferant(sachNrLieferantModul.getSachNrLieferant()).isEmpty()){
             ...
             schrittManager.addNewSchritt(sachNrLieferantModul, variante1, variante2, variante3, variante4, seite,
                     schritt, popis, farba, isCheck, bandName,user, assLevelSeq);
             schrittList=schrittManager.getAll();
        ...
        else{ Messagebox.show("Nezadali ste sch nr., lief nr.!, stranu, pas alebo schritt", "Error", Messagebox.OK, Messagebox.ERROR);}
    }
    @Command
    ...
    public Integer getAssLevelSeq() {
        return assLevelSeq;
    }

    public void setAssLevelSeq(Integer assLevelSeq) {
        this.assLevelSeq = assLevelSeq;
    }
}
