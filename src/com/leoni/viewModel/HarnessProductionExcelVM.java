...
/**
 * Created by hrmi1005 on 12. 2. 2016.
 */
...
public class HarnessProductionExcelVM {
    ...
    private String user;
    private List<String> modulesInProdNr;
    ...
    public void submit(@BindingParam("event") UploadEvent event)
    {
        Media media = event.getMedia();
        List<Moduls> modulsList = excelDocumentManager.getModulsFromExcel(media);
           ...
           // modulesInProdNr = lpab64Manager.findByProdnrAndKabelsatz(prodNr,kabelsatzKz);
            ...
            ausfuehrung = excelDocumentManager.getAusf(media);
            //kabelsatzKz = excelDocumentManager.getKabelsatzFromExcel(media);
            Integer modulsId;
            ...
            for(Moduls item : modulsList){
                if(true){
                    ...
                    //-----KLIPY---------------------------------------------------------------------------------------
                    //-----ZAPENOVACKA---------------------------------------------------------------------------------------
                    if(!foamManager.findBySachNrLieferant(item.getSachNrLieferant()).isEmpty()){
                        foamModules.add(foamManager.findBySachNrLieferant(item.getSachNrLieferant()).get(0));
                    }
                    //-----ZAPENOVACKA---------------------------------------------------------------------------------------
                    //-----OPTOAUTOMAT---------------------------------------------------------------------------------------
        ...
        variantToShow = variantManager.getSingleVariantFromMap(modulsIdSet,"F9X1RL_GRUND");*/
            if(variantToShow != null && ausfuehrung.trim().equals("RL")) {
                for(Variant variant:variantToShow){
                    hlreVariant = variant.getDescription()+"     "+variant.getScanString();}
                variantToShow = null;
            }

            variantToShow = variantManager.getSingleListVariantFromMap(modulsIdSet,"GRUND-HL-LI");
            if(variantToShow != null) {
                for(Variant variant:variantToShow){
            ...
            variantToShow = variantManager.getSingleListVariantFromMap(modulsIdSet,"GRUND-RELAISTRAEGER");
            if(variantToShow != null) {
                for(Variant variant:variantToShow){
        ...
        if(variantToShow != null) {hlreVariant = variantToShow.getDescription()+"     "+variantToShow.getScanString();
                variantToShow = null;}  */
            variantToShow = variantManager.getSingleListVariantFromMap(modulsIdSet,"DME-DME-SW");
            if(variantToShow != null) {
                for(Variant variant:variantToShow){
                    dmeSwVariant = variant.getDescription()+"     "+variant.getScanString();}
                variantToShow = null;}
            variantToShow = variantManager.getSingleListVariantFromMap(modulsIdSet,"DME-DME-GR");
            if(variantToShow != null) {
                for(Variant variant:variantToShow){
            ...
            if(variantToShow != null && step8A != null) {
                ...
                variantToShow = null;}
            variantToShow = variantManager.getSingleListVariantFromMap(modulsIdSet,"RM4B");
