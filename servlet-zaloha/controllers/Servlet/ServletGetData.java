...
public class ServletGetData {
   ...
   /* @RequestMapping(value = "/getDmeSwImgFromProdNr", method = RequestMethod.GET, produces = MediaType.IMAGE_JPEG_VALUE)
    ...
    /*@RequestMapping(value = "/isModulListDifficult")
    @ResponseBody
    public Boolean isModulListDifficult(@RequestParam("modulListStr") String modulListStr) {
        StringTokenizer st = new StringTokenizer(modulListStr, "#");
        String modulString = null;
        Moduls modul = null;
        boolean hasDiffModul = false;
        while (st.hasMoreElements()) {
            modulString = (String) st.nextElement();
            if (!modulsManager.findBySachNrLieferant(modulString.trim()).isEmpty()){
               modul = modulsManager.findBySachNrLieferant(modulString.trim()).get(0);
               if (modul.getDifficulty() != null && modul.getDifficulty().equals(3)){
                  hasDiffModul = true;
               }
            };
        }
    ...
    public String setAntennaStatus(@RequestParam("prodNr") String prodNr, @RequestParam("ksKz") String ksKz) {
        ...
        if(!lpab62Manager.findByProdnrAndKabelsatz(prodNr.trim(), ksKz.trim()).isEmpty()){
            ...
            prodNrLogManager.saveProdNrLog(lpab62.getProdNr(),lpab62.getKabelsatzKz(),"F9X1_ANTENNA","00000","ANTENNA STATUS","Antenna photo has been saved in archive");
        }
         return dateStr;
    }
    */
