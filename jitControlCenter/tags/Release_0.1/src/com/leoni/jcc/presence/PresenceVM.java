...
public class PresenceVM
    ...
    private void rebuildWorkplaceEditor()
        {
        List<Workplace> lw = new ArrayList<Workplace>();
        List<Place> pl = new ArrayList<Place>();
        pl.add(new Place("A1", "Dano"));
        pl.add(new Place("A2", "Matej"));
        pl.add(new Place("A3", "Oskar"));
        lw.add(new Workplace("1A", "Grund", pl));
        lw.add(new Workplace("2A", "Tulle_Bug", pl));

        Workplaces w = new Workplaces(lw);
        String s = XmlHandler.marshall(w);
        Workplaces wo = XmlHandler.unmarshall(s);

        List<String> listboxChose = new ArrayList<String>();
        listboxChose.add("A1");
        listboxChose.add("A2");
        listboxChose.add("A3");
...
//        siteAHolder.appendChild(ComponentFactory.getWorkplaceGroupbox(lw.get(0)));
//        siteAHolder.appendChild(ComponentFactory.getWorkplaceGroupbox(lw.get(1)));
        }
